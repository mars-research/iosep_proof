...
// in <value>
function method MapCreateFromSet<T(!new), U>(m:map<T, U>, s:set<T>, value:U) : (m':map<T, U>)
    ...
    decreases s
{
    if(s == {}) then
        ...
        MapCreateFromSet(m_step, s_step, value)
}
...
// Convert a set to a sequence
function SetToSeq_Func<T(!new)>(s:set<T>) : (q:seq<T>)
    ensures (forall e :: e in s <==> e in q)
    ensures (forall i, j :: 0 <= i < |q| && 0 <= j < |q|
                ==> (i == j <==> q[i] == q[j]))
    ensures (|s| == |q|)
...
function method SetSubstract<T>(s1:set<T>, s2:set<T>) : set<T>
    ensures SetSubstract<T>(s1, s2) <= s1
    ensures forall e :: e in s2 ==> e !in SetSubstract<T>(s1, s2)
{
    s1 - s2
}
...
//******** Utility Lemmas ********//
lemma Lemma_SetToSeq_Func_Property<T>(s:set<T>)
    ensures SeqToSet(SetToSeq_Func(s)) == s
{
...
lemma Lemma_SeqInsertFirstCorrectnessProperties<T>(a:seq<T>, b:seq<T>, e:T)
    requires a == [e] + b
    ensures |a| > 0
    ensures forall i :: 0<i<|a| - 1 ==> a[i] == b[i-1]
{
    // Danfy can automatically prove this lemma
}
...
lemma Lemma_SetMinusProperty2<T>(a:set<T>, b:set<T>, c:set<T>)
    requires b * c == {}
...
lemma Lemma_SetMinusProperty3<T>(a:set<T>, b:set<T>, c:set<T>)
    requires a * b == {}
    requires a * c == {}

    ensures a + b - c == a + (b - c)
{
    Lemma_SetsHaveEmptyIntersectionHasNoCommonElems(a, b);
    Lemma_SetsHaveEmptyIntersectionHasNoCommonElems(a, c);
}