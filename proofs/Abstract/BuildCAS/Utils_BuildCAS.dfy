...
predicate IsValidState_TDsToAllStates(k:State)
{
    ...
    (forall td_ids :: td_ids in k.tds_to_all_states <==> td_ids <= AllTDIDs(k.objects)) &&
...
{
    ReachableTDsKParams(k.subjects, AllTDIDs(k.objects), AllFDIDs(k.objects), AllDOIDs(k.objects),
                                AllHCodedTDIDs(k.subjects), BuildMap_DevsToHCodedTDVals(k.subjects, k.objects), 
                                BuildMap_ObjIDsToPIDs(k.objects), AllActiveTDs(k))
}

lemma Lemma_KToKParams_Properties(k:State, k_params:ReachableTDsKParams)
    requires IsValidState_Objects_UniqueObjIDs(k.objects)
        // Requirement: Objects have different internal IDs
    ...
    ensures k_params.subjects == k.subjects
...
{
    ActiveTDsState_SlimState(k.objects)[td_id]
}
