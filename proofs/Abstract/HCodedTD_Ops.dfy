...
function method BuildMap_DevsToHCodedTDVals(k_subjects:Subjects, k_objects:Objects) : (result:map<Dev_ID, TD_Val>)
    ...
    requires forall dev_id :: dev_id in k_subjects.devs
                ==> k_subjects.devs[dev_id].hcoded_td_id in k_objects.hcoded_tds

    ensures MapGetKeys(result) == MapGetKeys(k_subjects.devs)
    ensures forall dev_id :: dev_id in k_subjects.devs
                ==> result[dev_id] == k_objects.hcoded_tds[k_subjects.devs[dev_id].hcoded_td_id].val
    ensures var k_tds := TDState_FromState(k_objects);
            forall dev_id :: dev_id in k_subjects.devs
                ==> result[dev_id] == k_tds[k_subjects.devs[dev_id].hcoded_td_id].val
{
    map dev_id | dev_id in k_subjects.devs
               :: k_objects.hcoded_tds[k_subjects.devs[dev_id].hcoded_td_id].val
}
...
lemma Lemma_TDsDevHCodedRead_ReturnsAllTDsReadRefedInHCodedTD(
    ...
    requires forall dev_id, td_id :: 
                ...
                ==> td_id in AllTDIDs(k_objects)

    requires IDToDev_SlimState(k_subjects, dev_id).td_ids <= AllTDIDs(k_objects)
    requires IDToDev_SlimState(k_subjects, dev_id).fd_ids <= AllFDIDs(k_objects)
    requires IDToDev_SlimState(k_subjects, dev_id).do_ids <= AllDOIDs(k_objects)
    ...
    ensures TDsRefedByDevHCodedTDWithR(hcoded_tds, dev_id) <= AllTDIDs(k_objects)
...
lemma Lemma_TDsDevHCodedWrite_ReturnsAllTDsWriteRefedInHCodedTD(
    ...
    ensures TDsDevHCodedWrite(hcoded_tds, dev_id) <= AllTDIDs(k_objects)
    ensures forall o_id :: o_id in AllObjsIDs(k_objects) &&
                TD_ID(o_id) in IDToDev_SlimState(k_subjects, dev_id).td_ids &&
                DoObjRefedInDevHCodedTD(hcoded_tds, dev_id, o_id) &&
