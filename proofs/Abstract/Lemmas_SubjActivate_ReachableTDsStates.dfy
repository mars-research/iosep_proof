...
lemma Lemma_SubjObjActivate_TDsStateIsValidAfterAddTDsIffValidBefore_ActiveTDsInKAndNewKHaveNoInvalidTransfersInBothActiveTDsStates(
    ...
    requires forall dev_id2 :: dev_id2 in k.subjects.devs
                ==> DevHCodedTDRefsObjsInSameDev(k, dev_id2) &&
                    IDToDev(k, dev_id2).td_ids <= AllTDIDs(k.objects)
    requires forall o_id  :: TD_ID(o_id) in AllTDIDs(k.objects)
        ...
        // Property: In <k'_tds_state>, k'_td_id has no invalid transfers
{
    ...
    forall fd_id2 | fd_id2 in k'_tds_state[k'_td_id].trans_params_fds
        ensures fd_id2 in k'_params.objs_fd_ids
        ensures (ObjPID_SlimState(k'_params.objs_pids, fd_id2.oid) == 
                    ObjPID_SlimState(k'_params.objs_pids, k'_td_id.oid) ||
    ...
    {
        assert fd_id2 in k_tds_state[k'_td_id].trans_params_fds;

        if (ObjPID_SlimState(k_params.objs_pids, fd_id2.oid) == ObjPID_SlimState(k_params.objs_pids, k'_td_id.oid))
        {
            assert ObjPID_SlimState(k'_params.objs_pids, k'_td_id.oid) == ObjPID_SlimState(k_params.objs_pids, k'_td_id.oid);
            
            if(ObjPID_SlimState(k'_params.objs_pids, fd_id2.oid) != ObjPID_SlimState(k_params.objs_pids, fd_id2.oid))
            {
                assert ObjPID_SlimState(k_params.objs_pids, fd_id2.oid) != NULL;
                assert ObjStateUnchanged_FD(k.objects, k'.objects, fd_id2);
                assert false;
            }
            assert ObjPID_SlimState(k'_params.objs_pids, fd_id2.oid) == ObjPID_SlimState(k'_params.objs_pids, k'_td_id.oid);
        }
    }

    forall do_id2 | do_id2 in k'_tds_state[k'_td_id].trans_params_dos
        ensures do_id2 in k'_params.objs_do_ids
        ensures (ObjPID_SlimState(k'_params.objs_pids, do_id2.oid) == 
                    ObjPID_SlimState(k'_params.objs_pids, k'_td_id.oid) ||
                        // The TD (<k'_td_id>) contains a transfer, the target FD 
                        // (<do_id2>) must be in the same partition with the TD
                 k'_tds_state[k'_td_id].trans_params_dos[do_id2].amodes == {})
    {
        assert do_id2 in k_tds_state[k'_td_id].trans_params_dos;

        if (ObjPID_SlimState(k_params.objs_pids, do_id2.oid) == ObjPID_SlimState(k_params.objs_pids, k'_td_id.oid))
        {
            assert ObjPID_SlimState(k'_params.objs_pids, k'_td_id.oid) == ObjPID_SlimState(k_params.objs_pids, k'_td_id.oid);
            
            if(ObjPID_SlimState(k'_params.objs_pids, do_id2.oid) != ObjPID_SlimState(k_params.objs_pids, do_id2.oid))
            {
                assert ObjPID_SlimState(k_params.objs_pids, do_id2.oid) != NULL;
                assert ObjStateUnchanged_DO(k.objects, k'.objects, do_id2);
                assert false;
            }
            assert ObjPID_SlimState(k'_params.objs_pids, do_id2.oid) == ObjPID_SlimState(k'_params.objs_pids, k'_td_id.oid);
        }
    }
}
...
lemma Lemma_SubjObjActivate_TDsStateIsValidAfterAddTDsIfValidBefore(
    ...
    requires forall td_id, dev_id:Dev_ID :: IsDevID_SlimState(k'.subjects, dev_id) &&
                ...
                ==> (td_id in k'_tds_state && 
                     k'_tds_state[td_id] == ActiveObjValue_TD(k'.objects, td_id))
    requires forall dev_id, td_id :: dev_id in AllActiveDevs(k') &&
                    td_id == k'.subjects.devs[dev_id].hcoded_td_id
                ==> td_id in k'_tds_state &&
                    k'_tds_state[td_id] == k'.objects.hcoded_tds[td_id].val
    ...
    requires SubjObjActivate_UnchangedStateVarsBetweenKandNewK(k, k')
{
    ...
    (forall td_id :: td_id in AllTDIDs(k.objects) && td_id !in toactivate_td_ids
    ...
    (forall fd_id :: fd_id in AllActiveFDs(k)
                ==> ObjStateUnchanged_FD(k.objects, k'.objects, fd_id)) &&
    (forall do_id :: do_id in AllActiveDOs(k)
                ==> ObjStateUnchanged_DO(k.objects, k'.objects, do_id)) &&
    ...
    requires SubjObjActivate_UnchangedStateVarsBetweenKandNewK(k, k')
{
    (forall dev_id2:Dev_ID, td_id2 :: IsDevID(k, dev_id2) && 
                ...
                ==> td_id2 in k'.objects.active_non_hcoded_tds &&
                    IsTDClearVal(k'.objects.active_non_hcoded_tds, td_id2)) &&
...
predicate {:opaque} P_ReachableTDsStatesFromActiveTDsStatesOfNewK_HCodedTDsOfDevsToBeActivatedAreUnmodified(
...
predicate P_ReachableTDsStatesFromActiveTDsStatesOfNewK_TDsRefedByHCodedTDsWithROfDevToBeActivatedAreUnmodified(
    ...
    requires forall tds_state2 :: tds_state2 in k'_tds_states
                ==> MapGetKeys(tds_state2) == k'_params.all_active_tds
{
    forall tds_state2, td_id, dev_id:Dev_ID :: IsDevID_SlimState(k'.subjects, dev_id) &&
                    ...
                    td_id in TDsRefedByDevHCodedTDWithR(k'_params.hcoded_tds, dev_id)
                ==> (td_id in tds_state2 && 
                     tds_state2[td_id] == ActiveObjValue_TD(k'.objects, td_id))
...
lemma Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK(
    ...
    requires P_Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK(
        k, k', k'_params, toactivate_s_ids)
        // Requirement: In k'.objects.tds, TDs refed by hardcoded TDs with R are empty
    ...
    decreases |k'_tds_states|
{
    ...
    else if(|k'_tds_states| == 2)
    {
        ...
        if(SubjPID_DevID(k, k'_dev) != NULL)
        {
            ...
            Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK_TwoStates_ProveHCodedTDsOfDevsToBeActivatedAreUnmodified(
                k', k'_params, toactivate_s_ids, k'_tds_states);
        ...
        else
        {
            ...
            //Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_TwoStates_ProvePreConditionsForProperty2_ProvePreCondition(
            //    k, k', k'_params, toactivate_s_ids);
            Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_TwoStates_ProvePreConditionsForProperty2(
                k, k', k'_params, k'_dev, toactivate_s_ids, k'_tds_states, k'_from_tds_state);
    ...
    else
    {
        ...
        var next_k'_tds_states := k'_tds_states[..|k'_tds_states|-1];
        Lemma_SequenceHighlightLastElemOfSubSeq(k'_tds_states, next_k'_tds_states);
        assert next_k'_tds_states[|next_k'_tds_states|-1] == k'_tds_states[|k'_tds_states|-2];
...
{
    reveal P_HCodedTDsOfActiveDevsAreUnmodified();
    reveal P_ReachableTDsStatesFromActiveTDsStatesOfNewK_HCodedTDsOfDevsToBeActivatedAreUnmodified();
...
lemma Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK_Private(
    ...
    requires forall tds_state2 :: tds_state2 in k'_tds_states[..|k'_tds_states|-1]
                ==> MapGetKeys(tds_state2) == k'_params.all_active_tds
        // A property can be proven with the property above, yet not discovered by Dafny
    requires P_Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK(
        k, k', k'_params, toactivate_s_ids)
    ...
    decreases |k'_tds_states|
{
    ...
    else
    {
        ...
        // Prove property 2
        Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK_Private_ProvePreConditions(
            k, k', k'_params, toactivate_s_ids, toactivate_td_ids, k'_tds_states, k'_last_from_tds_state, k'_last_dev);
        Lemma_SubjObjActivate_TDsRefedByHCodedTDWithROfDevBeingActivatedAreEmpty(
            k, k', k'_params, toactivate_s_ids, k'_tds_states, k'_last_from_tds_state, k'_last_dev);
    ...
    ensures !DoActiveTDIncludeTransfersToInvalidObjOrObjOutsidePartition(k'_params, k'_tds_state, k'_td_id)
{
    ...
    assert k'.objects.hcoded_tds[k'.subjects.devs[k'_dev_id].hcoded_td_id].val == k.objects.hcoded_tds[k.subjects.devs[k'_dev_id].hcoded_td_id].val;
    assert k'.objects.hcoded_tds[k'_td_id].val == k.objects.hcoded_tds[k'_td_id].val;

    assert k.objects.hcoded_tds[k.subjects.devs[k'_dev_id].hcoded_td_id].val == HCodedTDValOfDev(KToKParams(k).hcoded_tds, k'_dev_id);
    assert k.objects.hcoded_tds[k.subjects.devs[k'_dev_id].hcoded_td_id].val == HCodedTDValOfDev(KToKParams(k').hcoded_tds, k'_dev_id);
    ...
    ensures DevModifyTDsStateOnlyWithHCodedWToTDs(KToKParams(k), dev_id, from_tds_state, to_tds_state)
{
    ...
    assert forall dev_id :: dev_id in k.subjects.devs
                ==> k_params.hcoded_tds[dev_id] == 
                    k.objects.hcoded_tds[k.subjects.devs[dev_id].hcoded_td_id].val;

    assert dev_id in AllActiveDevs(k);
    assert forall dev_id2, td_id2 :: dev_id2 in AllActiveDevs(k) &&
                    td_id2 == k.subjects.devs[dev_id2].hcoded_td_id
                ==> td_id2 in from_tds_state &&
                    from_tds_state[td_id2] == k.objects.hcoded_tds[td_id2].val;
                    ...
                    k', k'_params, toactivate_s_ids, k'_tds_states)
{
        ...
        ensures P_HCodedTDsOfActiveDevsAreUnmodified(k', tds_state2)
    {
        if(tds_state2 == k'_last_to_tds_state)
        {
            forall dev_id, td_id | dev_id in AllActiveDevs(k') &&
                ...
                ensures tds_state2[td_id] == k'.objects.hcoded_tds[td_id].val
            {
                     ...
                     tds_state2[td_id] == ActiveObjValue_TD(k'.objects, td_id))
    {
        if(tds_state2 == k'_last_to_tds_state)
        {
            ...
            assert tds_state2[td_id] == ActiveObjValue_TD(k'.objects, td_id);
        }
    }
}
                    ...
                    k', k'_params, toactivate_s_ids, k'_tds_states)
{
        ...
        ensures P_HCodedTDsOfActiveDevsAreUnmodified(k', tds_state2)
    {
        ...
        forall dev_id, td_id | dev_id in AllActiveDevs(k') &&
                    td_id == k'.subjects.devs[dev_id].hcoded_td_id
            ensures td_id in tds_state2 
            ensures td_id in k'.objects.hcoded_tds
            ensures tds_state2[td_id] == k'.objects.hcoded_tds[td_id].val
        {
...
lemma Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK_TwoStates_ProveHCodedTDsOfDevsToBeActivatedAreUnmodified(
    k':State, k'_params:ReachableTDsKParams,
    toactivate_s_ids:set<Subject_ID>, k'_tds_states:seq<TDs_Vals>
)
    requires IsValidState_Objects_UniqueObjIDs(k'.objects)
...
{
    reveal P_ReachableTDsStatesFromActiveTDsStatesOfNewK_HCodedTDsOfDevsToBeActivatedAreUnmodified();
    forall tds_state2 | tds_state2 in k'_tds_states[..|k'_tds_states|-1]
        ensures P_HCodedTDsOfActiveDevsAreUnmodified(k', tds_state2)
    {
        reveal P_HCodedTDsOfActiveDevsAreUnmodified();
        forall dev_id, td_id | dev_id in AllActiveDevs(k') &&
...
predicate P_Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK(
    k:State, k':State, k'_params:ReachableTDsKParams, toactivate_s_ids:set<Subject_ID>
)
    requires IsValidState_Objects_UniqueObjIDs(k'.objects)
        // Requirement: Objects have different internal IDs
    requires SubjObjActivate_UnchangedStateVarsBetweenKandNewK(k, k')
...
lemma Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_TwoStates_ProvePreConditionsForProperty2(
    k:State, k':State, k'_params:ReachableTDsKParams, k'_dev:Dev_ID,
    toactivate_s_ids:set<Subject_ID>, k'_tds_states:seq<TDs_Vals>,
    k'_from_tds_state:TDs_Vals
)
    requires IsValidState_Objects_UniqueObjIDs(k'.objects)
        // Requirement: Objects have different internal IDs
    requires SubjObjActivate_UnchangedStateVarsBetweenKandNewK(k, k')
    ...
    requires |k'_tds_states| == 2
    requires k'_from_tds_state == k'_tds_states[0]
    requires k'_tds_states[0] == ActiveTDsState(k')
    ...
    requires IsDevID_SlimState(k'.subjects, k'_dev)
    requires k'_dev.sid in toactivate_s_ids

    requires forall td_id :: td_id in k'.subjects.devs[k'_dev].td_ids
                ==> ObjPID(k', td_id.oid) != NULL

    ensures P_HCodedTDsOfActiveDevsAreUnmodified(k', k'_from_tds_state)
    ensures forall td_id:TD_ID :: td_id in TDsRefedByDevHCodedTDWithR(k'_params.hcoded_tds, k'_dev)
                ==> td_id in k'_from_tds_state &&
                    k'_from_tds_state[td_id].trans_params_tds == map[] &&
                    k'_from_tds_state[td_id].trans_params_fds == map[] &&
                    k'_from_tds_state[td_id].trans_params_dos == map[]
{
    // Prove property 1
    reveal P_HCodedTDsOfActiveDevsAreUnmodified();
    forall dev_id, td_id | dev_id in AllActiveDevs(k') &&
        ...
        ensures td_id in k'_from_tds_state 
        ensures td_id in k'.objects.hcoded_tds
        ensures k'_from_tds_state[td_id] == k'.objects.hcoded_tds[td_id].val
    {
        assert ObjPID(k', td_id.oid) != NULL;
    }

    // Prove property 2
    forall td_id:TD_ID | td_id in TDsRefedByDevHCodedTDWithR(k'_params.hcoded_tds, k'_dev)
        ensures td_id in k'_from_tds_state
        ensures k'_from_tds_state[td_id].trans_params_tds == map[]
        ensures k'_from_tds_state[td_id].trans_params_fds == map[]
        ensures k'_from_tds_state[td_id].trans_params_dos == map[]
    {
        assert td_id in IDToDev_SlimState(k'_params.subjects, k'_dev).td_ids;
        assert DoOwnObj_SlimState(k'_params.subjects, k'_dev.sid, td_id.oid);
        assert TDsStateGetTDIDs(k'_from_tds_state) == k'_params.all_active_tds;
    }
}

lemma Lemma_SubjObjActivate_TDsRefedByHCodedTDWithROfDevBeingActivatedAreEmpty(
    k:State, k':State, k'_params:ReachableTDsKParams,
    toactivate_s_ids:set<Subject_ID>, k'_tds_states:seq<TDs_Vals>,
...
lemma Lemma_SubjObjActivate_IfTDsStatesCanBeReachedFromActiveTDsStatesInNewK_ThenReducedTDsStatesCanBeReachedInK_Private_ProvePreConditions(
    k:State, k':State, k'_params:ReachableTDsKParams,
    toactivate_s_ids:set<Subject_ID>, toactivate_td_ids:set<TD_ID>, k'_tds_states:seq<TDs_Vals>,
    k'_last_from_tds_state:TDs_Vals, k'_last_dev:Dev_ID
)
    requires IsValidState_Objects_UniqueObjIDs(k'.objects)
        // Requirement: Objects have different internal IDs
    ...
    ensures P_HCodedTDsOfActiveDevsAreUnmodified(k', k'_last_from_tds_state)
{
    reveal P_ReachableTDsStatesFromActiveTDsStatesOfNewK_HCodedTDsOfDevsToBeActivatedAreUnmodified();

    assert P_ReachableTDsStatesFromActiveTDsStatesOfNewK_HCodedTDsOfDevsToBeActivatedAreUnmodified(
                    k', toactivate_s_ids, k'_tds_states[..|k'_tds_states|-1]);
    assert k'_last_from_tds_state == k'_tds_states[|k'_tds_states|-2];
    assert k'_last_from_tds_state in k'_tds_states[..|k'_tds_states|-1];
    assert P_HCodedTDsOfActiveDevsAreUnmodified(k', k'_last_from_tds_state);
}
...
predicate {:opaque} P_HCodedTDsOfActiveDevsAreUnmodified(k:State, tds_state:TDs_Vals)
    requires IsValidState_Objects_UniqueObjIDs(k.objects)
        // Requirement: Objects have different internal IDs
    requires SubjObjActivate_PreConditionsOfK(k)
{
    forall dev_id, td_id :: dev_id in AllActiveDevs(k) &&
                    td_id == k.subjects.devs[dev_id].hcoded_td_id
                ==> td_id in tds_state && td_id in k.objects.hcoded_tds &&
                    tds_state[td_id] == k.objects.hcoded_tds[td_id].val
...
{
    // Dafny can automatically prove this lemma
    assert MapGetKeys(k'.objects.hcoded_tds) == MapGetKeys(k.objects.hcoded_tds);
}
...
lemma Lemma_CanActiveDevReadActiveTD_SubjObjActivate_IfSubjsToActivateCanReadTDThenOwnTD(
    ...
    requires forall dev_id, td_id :: dev_id in AllActiveDevs(k) &&
                    td_id == k.subjects.devs[dev_id].hcoded_td_id
                ==> td_id in k_tds_state && td_id in k.objects.hcoded_tds &&
                    k_tds_state[td_id] == k.objects.hcoded_tds[td_id].val
            ...
            k_tds_state[k_td_id] == TD_Val(map[], map[], map[])
{
                ...
                k_tds_state[k_td_id] == TD_Val(map[], map[], map[])
    {
        if(|td_ids| > 2)
        {
            ...
            assert k_params.hcoded_tds[k_dev_id] == k.objects.hcoded_tds[k.subjects.devs[k_dev_id].hcoded_td_id].val;
    ...
    ensures tdx_id in k.subjects.devs[dev_id].td_ids
{
    ...
    if(tdx_id !in k.subjects.devs[dev_id].td_ids)
    {
        ...
        // Show conflicts
        assert P_HCodedTDsOfActiveDevsAreUnmodified(k, from_tds_state);
        reveal P_HCodedTDsOfActiveDevsAreUnmodified();
        assert from_tds_state[td_ids[0]] == k.objects.hcoded_tds[td_ids[0]].val;
        assert from_tds_state[td_ids[0]] == k_params.hcoded_tds[dev_id];

        assert td_ids[1] in TDsRefedByDevHCodedTDWithR(k_params.hcoded_tds, dev_id);
