...
{
    (IsValidState(k) && IsSecureState(k)) &&
    (td_ids <= AllTDIDs(k.objects)) &&
    (fd_ids <= AllFDIDs(k.objects)) &&
    (do_ids <= AllDOIDs(k.objects)) &&
...
)
{
    ...
    (IDToDrv(k, Drv_ID(drv_sid)).pid != NULL) &&
        // Requirement: The driver is in the state and is active
    ((forall td_id :: td_id in td_id_val_map ==> td_id in AllTDIDs(k.objects)) &&
             (forall fd_id :: fd_id in fd_id_val_map ==> fd_id in AllFDIDs(k.objects)) &&
             (forall do_id :: do_id in do_id_val_map ==> do_id in AllDOIDs(k.objects))) &&
    (forall dev_id :: dev_id in k.subjects.devs
...
predicate P_K_SubjRead_ObjsToCopyFromAndToMustBeActive(
    k:State, tds_dst_src:map<TD_ID, TD_ID>, fds_dst_src:map<FD_ID, FD_ID>, dos_dst_src:map<DO_ID, DO_ID>
)
    requires IsValidState_Subjects(k) && IsValidState_Objects(k)
{
...
{
    (forall td_id :: td_id in tds_dst_src
                ==> td_id in AllTDIDs(k.objects) && tds_dst_src[td_id] in AllTDIDs(k.objects)) &&
    (forall fd_id :: fd_id in fds_dst_src
                ==> fd_id in AllFDIDs(k.objects) && fds_dst_src[fd_id] in AllFDIDs(k.objects)) &&
    (forall do_id :: do_id in dos_dst_src
                ==> do_id in AllDOIDs(k.objects) && dos_dst_src[do_id] in AllDOIDs(k.objects)) &&
    ...
    requires DevRead_ReadSrcObjsToDestObjs_SrcAndDstObjsOfCopyMustBeInWSState(k, tds_dst_src, fds_dst_src, dos_dst_src)
{
    (forall td_id2 :: td_id2 in tds_dst_src
                ==> DevWrite_WriteTDWithValMustBeInATransfer(k, dev_sid, td_id2, DevRead_GetReadObjVal_AnyTD(k.objects, tds_dst_src[td_id2]))
    ) &&
    (forall fd_id2 :: fd_id2 in fds_dst_src
                ==> DevWrite_WriteFDWithValMustBeInATransfer(k, dev_sid, fd_id2, DevRead_GetReadObjVal_AnyFD(k.objects, fds_dst_src[fd_id2]))
    ) &&
    (forall do_id2 :: do_id2 in dos_dst_src
                ==> DevWrite_WriteDOWithValMustBeInATransfer(k, dev_sid, do_id2, DevRead_GetReadObjVal_AnyDO(k.objects, dos_dst_src[do_id2]))
    ) &&
...
// This function is for the predicate <DevRead_ReadSrcObjsToDestObjs_WriteInCopyMustFromCorrespondingTransfers>, and 
// hence this function temporially regards inactive objects also have values. Later the model proves that all objects
// to be read must be active, and hence this assumption does not affect the proof at all
function DevRead_GetReadObjVal_AnyTD(k_objects:Objects, td_id:TD_ID) : TD_Val
    requires td_id in AllTDIDs(k_objects)
{
    ...
    else if(td_id in k_objects.hcoded_tds) then
...
// hence this function temporially regards inactive objects also have values. Later the model proves that all objects
// to be read must be active, and hence this assumption does not affect the proof at all
function DevRead_GetReadObjVal_AnyFD(k_objects:Objects, fd_id:FD_ID) : FD_Val
    requires fd_id in AllFDIDs(k_objects)
{
    if(fd_id in k_objects.active_fds) then
        k_objects.active_fds[fd_id].val
    else
        FD_Val("")
}
...
// hence this function temporially regards inactive objects also have values. Later the model proves that all objects
// to be read must be active, and hence this assumption does not affect the proof at all
function DevRead_GetReadObjVal_AnyDO(k_objects:Objects, do_id:DO_ID) : DO_Val
    requires do_id in AllDOIDs(k_objects)
{
    if(do_id in k_objects.active_dos) then
        k_objects.active_dos[do_id].val
    else
        DO_Val("")
}
