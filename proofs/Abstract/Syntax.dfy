...
datatype FD_Val              = FD_Val(v:string)
datatype DO_Val              = DO_Val(v:string)
...
datatype Objects             = Objects(active_non_hcoded_tds:map<TD_ID, TD_State>,
                                active_fds:map<FD_ID, FD_State>,
                                active_dos:map<DO_ID, DO_State>,

                                // The states only need to remember the IDs of inactive objects, but not their values
                                inactive_non_hcoded_tds:set<TD_ID>,
                                inactive_fds:set<FD_ID>,
                                inactive_dos:set<DO_ID>,

                                // The model stores the value of hardcoded TDs, besides of their PIDs
                                // [NOTE] These values only shows hardcoded TDs have constant values when they are 
                                // active. When they are inactive, their values are meaningless, similar to other 
                                // inactive objects
                                hcoded_tds:map<TD_ID, TD_State>
                                )
