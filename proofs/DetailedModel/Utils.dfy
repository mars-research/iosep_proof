...
function method DM_AllDrvIDs(ws_subjects:Subjects) : set<Drv_ID>
{
    MapGetKeys(ws_subjects.drvs)
}
...
{
    IsValidState_Objects_UniqueObjIDs(ws_objects)
}
...
function method DM_ObjPID(ws_objects:Objects, o_id:Object_ID) : Partition_ID
    ...
    ensures (TD_ID(o_id) in AllTDIDs(ws_objects) ==> BuildMap_ObjIDsToPIDs(ws_objects)[o_id] == ObjPID_KObjects(ws_objects, o_id)) &&
            (FD_ID(o_id) in AllFDIDs(ws_objects) ==> BuildMap_ObjIDsToPIDs(ws_objects)[o_id] == ObjPID_KObjects(ws_objects, o_id)) &&
            (DO_ID(o_id) in AllDOIDs(ws_objects) ==> BuildMap_ObjIDsToPIDs(ws_objects)[o_id] == ObjPID_KObjects(ws_objects, o_id))
{
...
function method DM_AllActiveTDs(ws_objects:Objects) : set<TD_ID>
    requires P_DMObjectsHasUniqueIDs(ws_objects)
{
    AllActiveTDs_SlimState(ws_objects)
}
...
function method DM_TDIDsInGreen(ws:DM_State) : (result:set<TD_ID>)
    requires P_DMObjectsHasUniqueIDs(ws.objects)

    ensures forall td_id :: td_id in DM_AllTDIDs(ws.objects) &&
...
function method DM_IsTDClearVal(ws_objects:Objects, td_id:TD_ID) : bool
    ...
    requires td_id in ws_objects.active_non_hcoded_tds
{
    IsTDClearVal(ws_objects.active_non_hcoded_tds, td_id)
}

function method DM_IsFDClearVal(ws_objects:Objects, fd_id:FD_ID) : bool
    requires P_DMObjectsHasUniqueIDs(ws_objects)
    
    requires fd_id in ws_objects.active_fds
{
    IsFDClearVal(ws_objects.active_fds, fd_id)
}

function method DM_IsDOClearVal(ws_objects:Objects, do_id:DO_ID) : bool
    requires P_DMObjectsHasUniqueIDs(ws_objects)
    
    requires do_id in ws_objects.active_dos
{
    IsDOClearVal(ws_objects.active_dos, do_id)
}

function DM_IsSameTD(ws_objects':Objects, ws_objects:Objects, td_id:TD_ID) : bool
    requires P_DMObjectsHasUniqueIDs(ws_objects')
    
    requires AllTDIDs(ws_objects') == AllTDIDs(ws_objects)
    requires td_id in DM_AllTDIDs(ws_objects)
{
    ObjStateUnchanged_TD(ws_objects, ws_objects', td_id)
}

function DM_IsSameFD(ws_objects':Objects, ws_objects:Objects, fd_id:FD_ID) : bool
    requires P_DMObjectsHasUniqueIDs(ws_objects')
    
    requires AllFDIDs(ws_objects') == AllFDIDs(ws_objects)
    requires fd_id in DM_AllFDIDs(ws_objects)
{
    ObjStateUnchanged_FD(ws_objects, ws_objects', fd_id)
}

function DM_IsSameDO(ws_objects':Objects, ws_objects:Objects, do_id:DO_ID) : bool
    requires P_DMObjectsHasUniqueIDs(ws_objects')
    
    requires AllDOIDs(ws_objects') == AllDOIDs(ws_objects)
    requires do_id in DM_AllDOIDs(ws_objects)
{
    ObjStateUnchanged_DO(ws_objects, ws_objects', do_id)
}
...
lemma Lemma_DM_AllObjsIDs_Property()
    ensures forall objs1:Objects, objs2:Objects ::
            (AllTDIDs(objs1) == AllTDIDs(objs2) &&
             AllFDIDs(objs1) == AllFDIDs(objs2) &&
             AllDOIDs(objs1) == AllDOIDs(objs2))
...
function method DM_DrvDevRead_ReplaceSrcTDWithVal(
    ...
    ensures forall td_id :: td_id in td_id_val_map
                ==> td_id_val_map[td_id] == ws.objects.active_non_hcoded_tds[tds_dst_src[td_id]].val

    ensures DM_DrvDevRead_ReplaceSrcTDWithVal(ws, tds_dst_src) == DrvDevRead_ReplaceSrcTDWithVal(DMStateToState(ws), tds_dst_src)
{
    map td_id | td_id in tds_dst_src :: ws.objects.active_non_hcoded_tds[tds_dst_src[td_id]].val
}
...
function method DM_DrvDevRead_ReplaceSrcFDWithVal(
    ...
    ensures forall fd_id :: fd_id in fd_id_val_map
                ==> fd_id_val_map[fd_id] == ws.objects.active_fds[fds_dst_src[fd_id]].val

    ensures DM_DrvDevRead_ReplaceSrcFDWithVal(ws, fds_dst_src) == DrvDevRead_ReplaceSrcFDWithVal(DMStateToState(ws), fds_dst_src)
{
    map fd_id | fd_id in fds_dst_src :: ws.objects.active_fds[fds_dst_src[fd_id]].val
}
...
function method DM_DrvDevRead_ReplaceSrcDOWithVal(
    ...
    ensures forall do_id :: do_id in do_id_val_map
                ==> do_id_val_map[do_id] == ws.objects.active_dos[dos_dst_src[do_id]].val

    ensures DM_DrvDevRead_ReplaceSrcDOWithVal(ws, dos_dst_src) == DrvDevRead_ReplaceSrcDOWithVal(DMStateToState(ws), dos_dst_src)
{
    map do_id | do_id in dos_dst_src :: ws.objects.active_dos[dos_dst_src[do_id]].val
}

lemma Lemma_DM_DrvDevRead_ReplaceSrcTDWithVal_Property_EmptyWriteList(dm:DM_State)
    ensures map[] == DM_DrvDevRead_ReplaceSrcTDWithVal(dm, map[])
{
    // Dafny can automatically prove this lemma
}

lemma Lemma_DM_DrvDevRead_ReplaceSrcFDWithVal_Property_EmptyWriteList(dm:DM_State)
    ensures map[] == DM_DrvDevRead_ReplaceSrcFDWithVal(dm, map[])
{
    // Dafny can automatically prove this lemma
}

lemma Lemma_DM_DrvDevRead_ReplaceSrcDOWithVal_Property_EmptyWriteList(dm:DM_State)
    ensures map[] == DM_DrvDevRead_ReplaceSrcDOWithVal(dm, map[])
{
    // Dafny can automatically prove this lemma
}