...
lemma Lemma_DM_SI1HoldsIfRedTDsAreUnchanged(
    ...
    requires AllFDIDs(ws'.objects) == AllFDIDs(ws.objects)
    requires AllDOIDs(ws'.objects) == AllDOIDs(ws.objects)
    ...
    requires DM_IsSecureState_SI2(ws)
    requires ActiveTDsStateIsSecure(KToKParams(k), AllActiveDevs(k), ActiveTDsState_SlimState(k.objects))
    ...
    ensures DM_IsSecureState_SI1(ws')
{
    ...
    var k'_tds_state := ActiveTDsState_SlimState(k'.objects);
...
lemma Lemma_RedTDsStateIsSame_IfRedTDsAreUnmodified(ws:DM_State, k:State, ws':DM_State, k':State, k_os_tds_state0:TDs_Vals, k'_os_tds_state0:TDs_Vals)
    ...
    requires k_os_tds_state0 == MaskTDsState(ActiveTDsState_SlimState(k.objects), DM_TDIDsInGreen(ws))
    requires k'_os_tds_state0 == MaskTDsState(ActiveTDsState_SlimState(k'.objects), DM_TDIDsInGreen(ws'))
...
predicate P_Lemma_AllTDsStatesInKOSTDsStatesAreSecure_Property1(
    ws:DM_State, k:State, k_params:ReachableTDsKParams,
    ...
    requires k == DMStateToState(ws)
    requires k_params == KToKParams(k)
    requires k_tds_state == ActiveTDsState_SlimState(k.objects)
...
predicate P_Lemma_AllTDsStatesInKOSTDsStatesAreSecure_Properties(
    ...
    requires forall tds_state2 :: tds_state2 in k_os_tds_states 
                ==> TDsStateGetTDIDs(tds_state2) == AllActiveTDs(k)
{
    P_Lemma_AllTDsStatesInKOSTDsStatesAreSecure_Property1(ws, k, k_params, k_os_tds_states, k_tds_state, k_wimps_tds_state0, red_devs) &&
...
lemma Lemma_P_Lemma_AllTDsStatesInKOSTDsStatesAreSecure_Property1_Prove(
    ws:DM_State, k:State, k_params:ReachableTDsKParams,
    k_os_tds_states:seq<TDs_Vals>, k_tds_state:TDs_Vals, k_wimps_tds_state0:TDs_Vals, 
    red_devs:set<Dev_ID>
)
    ...
    requires forall tds_state2 :: tds_state2 in k_os_tds_states
        ...
        // Requirement: k_tds_state -->* expanded_tds_state2

    ensures P_Lemma_AllTDsStatesInKOSTDsStatesAreSecure_Property1(ws, k, k_params, k_os_tds_states, k_tds_state, k_wimps_tds_state0, red_devs)
{
...
lemma Lemma_GreenDrvWrite_TDsInRedPartitionHasNoInvalidRefs_IfDMIsSecureState(
    ...
    requires DM_IsSecureState_SI1(ws)
    requires DM_IsSecureState_SI2(ws)
    requires ActiveTDsStateIsSecure(k_params, AllActiveDevs(k), ActiveTDsState_SlimState(k.objects))
                ...
                k_params, AllObjsIDs(k.objects), DM_DevsInRed(ws), k_os_tds_state0)
{
    ...
    var k_tds_state := ActiveTDsState_SlimState(k.objects);
    var k_wimps_tds_state0 := MaskTDsState(ActiveTDsState_SlimState(k.objects), DM_TDIDsInRed(ws));
...
lemma Lemma_AllTDsStatesInNewKGreenTDsStatesAreSecure_TwoOrMoreElems(
    ...
    requires k' == DMStateToState(ws')
    requires k'_params == KToKParams(k')
    requires k'_tds_state == ActiveTDsState_SlimState(k'.objects)
...
lemma Lemma_GreenDrvWrite_TDsInGreenPartitionHasNoInvalidRefs_IfDMIsSecureState(
    ...
    requires k' == DMStateToState(ws')
    requires k'_params == KToKParams(k')
    requires k'_wimps_tds_state0 == MaskTDsState(ActiveTDsState_SlimState(k'.objects), DM_TDIDsInRed(ws'))
                ...
                k'_params, AllObjsIDs(k'.objects), DM_DevsInGreen(ws'), k'_wimps_tds_state0)
{
    ...
    forall tds_state2:TDs_Vals | TDsStateGetTDIDs(tds_state2) == k'_params.all_active_tds &&
                        ...
                        (   IsActiveTDsStateReachActiveTDsStateInChain_PreConditions(k'_params, 
                            ...
                            IsActiveTDsStateReachActiveTDsStateInChain(k'_params,  
                                green_devs, k'_wimps_tds_state0, tds_state2)))
                                    // k_os_tds_state0 -->* tds_state2
        ensures forall td_id2, dev_id :: 
                        dev_id in green_devs && 
                            // The device (<dev_id>) is an active OS device
                        td_id2 in TDsStateGetTDIDs(tds_state2) &&
                            // The TD (<td_id2>) is active
                        CanActiveDevReadActiveTD_PreConditions(k'_params, tds_state2, dev_id, td_id2) &&
                        CanActiveDevReadActiveTD(k'_params, tds_state2, dev_id, td_id2)
                            // The TD is read by the device
                        ==> !DoActiveTDIncludeTransfersToInvalidObjOrObjOutsidePartition(k'_params, tds_state2, td_id2)
    {
        forall td_id, td_id2 | td_id in k'_wimps_tds_state0 && td_id2 in k'_wimps_tds_state0[td_id].trans_params_tds
            ensures W !in k'_wimps_tds_state0[td_id].trans_params_tds[td_id2].amodes
        {
            //assert k'_wimps_tds_state0[td_id].trans_params_tds[td_id2].amodes ==
             //       k'.objects.tds[td_id].val.trans_params_tds[td_id2].amodes;
            if(td_id in DM_TDIDsInGreen(ws'))
            {
                assert W !in k'_wimps_tds_state0[td_id].trans_params_tds[td_id2].amodes;
            }
            else
            {
                assert W !in k'_wimps_tds_state0[td_id].trans_params_tds[td_id2].amodes;
        ...
        // Proves tds_state2 == k'_wimps_tds_state0
        if(k'_wimps_tds_state0 != tds_state2)
        {
            ...
            Lemma_AllTDsStatesInNewKGreenTDsStatesAreSecure_TwoOrMoreElems(ws', k', k'_params,
                k'_wimps_tds_states, ActiveTDsState_SlimState(k'.objects),
        ...
        forall td_id2, dev_id | 
                        ...
                        CanActiveDevReadActiveTD_PreConditions(k'_params, tds_state2, dev_id, td_id2) &&
                        CanActiveDevReadActiveTD(k'_params, tds_state2, dev_id, td_id2)
            ensures !DoActiveTDIncludeTransfersToInvalidObjOrObjOutsidePartition(k'_params, tds_state2, td_id2)
        {
            
            if(td_id2 !in DM_TDIDsInGreen(ws'))
            {
                    // Prove td_id2 is not the hardcoded TD of <dev_id>
                    if(td_id2 == k'.subjects.devs[dev_id].hcoded_td_id)
                    {
                    ...
                    var td_ids:seq<TD_ID> := Lemma_CanActiveDevReadActiveTD_Apply(k'_params, tds_state2, dev_id, td_id2);
                    assert |td_ids| > 1;
                    var tdx_id:TD_ID := Lemma_K_IfActiveDevReadsTDIndirectly_ThenDevCanReadTDX(k', k'_params, tds_state2, td_ids, dev_id, td_id2);

                    assert tdx_id in AllActiveTDs(k') &&
                            CanActiveDevReadActiveTD(k'_params, tds_state2, dev_id, tdx_id) &&
                            //tdx_id !in k'_params.hcoded_td_ids &&
                            td_id2 in tds_state2[tdx_id].trans_params_tds &&
                            R in tds_state2[tdx_id].trans_params_tds[td_id2].amodes;
                                // The TD references the target TD (<td_id2>) with R
                    assert DoActiveTDIncludeTransfersToInvalidObjOrObjOutsidePartition(k'_params, tds_state2, tdx_id);
                    assert false;
            }
...
lemma Lemma_AllTDsStatesInKOSTDsStatesAreSecure_TwoOrMoreElems_Private(
    ...
    requires k == DMStateToState(ws)
    requires k_params == KToKParams(k)
    requires k_wimps_tds_state0 == MaskTDsState(ActiveTDsState_SlimState(k.objects), DM_TDIDsInRed(ws))
                ...
                k_os_tds_states, k_tds_state, k_wimps_tds_state0, red_devs)
{
    ...
    assert P_Lemma_AllTDsStatesInKOSTDsStatesAreSecure_Property1(ws, k, k_params, k_os_tds_states, k_tds_state, k_wimps_tds_state0, red_devs) by
    {
        forall tds_state2 | tds_state2 in k_os_tds_states   
            ensures var expanded_tds_state2 := MergeTDsState(tds_state2, k_wimps_tds_state0, DM_TDIDsInGreen(ws));
                    expanded_tds_state2 == k_tds_state ||
                    (IsActiveTDsStateReachActiveTDsStateInChain_PreConditions(k_params, 
                            red_devs, k_tds_state, expanded_tds_state2) &&
                        IsActiveTDsStateReachActiveTDsStateInChain(k_params,  
        ...
        Lemma_P_Lemma_AllTDsStatesInKOSTDsStatesAreSecure_Property1_Prove(ws, k, k_params, k_os_tds_states, k_tds_state, k_wimps_tds_state0, red_devs);
    }
