...
// Axiom: For any operations defined in this model, any state satisfies P_DM_OpsProperties
lemma {:axiom} Lemma_AllOpsInConcreteModel_SatisfyP_DM_OpsProperties()
    ensures forall ws:DM_State, op:DM_Op :: P_DM_OpsProperties(ws, op)
        // [Note] For any operations defined in this model, any state satisfies P_DM_OpsProperties
    ...
    requires AllTDIDs(ws'.objects) == AllTDIDs(ws.objects)
{
    ...
    ObjPID_KObjects(ws'.objects, td_id.oid) != ObjPID_KObjects(ws.objects, td_id.oid) &&
    ObjPID_KObjects(ws'.objects, td_id.oid) != NULL &&
    ...
    ensures forall t:DM_Trace :: |t.ops| > 0 &&
                ...
                ==> (forall i :: 0 <= i < |t.ops| 
                        ==> (forall dev_id :: dev_id in DM_CalcNewStates(t)[i].subjects.devs
                                ==> DM_CalcNewStates(t)[i].subjects.devs[dev_id].hcoded_td_id in DM_CalcNewStates(t)[i].objects.hcoded_tds) && // Needed by DM_IsSecureOps
                        ...
                        // Each transition in trace satisfies all TCs
{
    forall t:DM_Trace | |t.ops| > 0 &&
        ...
        ensures forall i :: 0 <= i < |t.ops| 
                    ==> (forall dev_id :: dev_id in DM_CalcNewStates(t)[i].subjects.devs
                                ==> DM_CalcNewStates(t)[i].subjects.devs[dev_id].hcoded_td_id in DM_CalcNewStates(t)[i].objects.hcoded_tds) &&
                        DM_IsSecureOps(DM_CalcNewStates(t)[i], DM_CalcNewStates(t)[i+1])
    {
        var states_seq := DM_CalcNewStates(t);
        forall i | 0 <= i < |t.ops|
            ensures forall dev_id :: dev_id in states_seq[i].subjects.devs
                                ==> states_seq[i].subjects.devs[dev_id].hcoded_td_id in states_seq[i].objects.hcoded_tds
...
predicate P_DM_OpsProperties(ws:DM_State, op:DM_Op)
{
    ...
    (op.DM_DrvActivateToRedPartitionOp? ==> P_DM_OpsProperties_DrvActivateToRedPartitionOp(ws, op)) &&
    (op.DM_GreenDrvDeactivateOp? ==> P_DM_OpsProperties_GreenDrvDeactivateOp(ws, op)) &&
    (op.DM_RedDrvDeactivateOp? ==> P_DM_OpsProperties_RedDrvDeactivateOp(ws, op)) &&
    ...
    (op.DM_ExternalObjsActivateToRedPartitionOp? ==> P_DM_OpsProperties_ExternalObjsActivateToRedPartitionOp(ws, op)) &&
    (op.DM_GreenExternalObjsDeactivateOp? ==> P_DM_OpsProperties_GreenExternalObjsDeactivateOp(ws, op)) &&
    (op.DM_RedExternalObjsDeactivateOp? ==> P_DM_OpsProperties_RedExternalObjsDeactivateOp(ws, op))
}
...
predicate P_DM_OpsFulfillPreConditions(ws:DM_State, op:DM_Op)
{
    ...
    (op.DM_DrvActivateToRedPartitionOp? ==> (DM_IsValidState(ws) && DM_IsSecureState(ws) &&
                        Drv_ID(op.drv_sid) in ws.subjects.drvs)) &&
    ...
    (op.DM_RedDrvDeactivateOp? ==> (DM_IsValidState(ws) && DM_IsSecureState(ws) &&
                        Drv_ID(op.drv_sid) in ws.subjects.drvs &&
                        DM_SubjPID(ws.subjects, op.drv_sid) == ws.red_pid &&
                        P_RedDevsHaveNoTransferToGivenRedDrvAtAnyTime(ws, Drv_ID(op.drv_sid)))) &&
    ...
    (op.DM_ExternalObjsActivateToRedPartitionOp? ==> DM_ExternalObjsActivateToRedPartition_PreConditions(ws, op.td_ids, op.fd_ids, op.do_ids)) &&
    ...
    (op.DM_RedExternalObjsDeactivateOp? ==> DM_RedExternalObjsDeactivate_PreConditions(ws, op.td_ids, op.fd_ids, op.do_ids))
}
    ...
    ensures DM_IsSecureOps(ws, result.0)
{
    if(op.DM_RedDrvReadOp?) then
        ...
        var ws', ws_d :| DM_GreenDrvRead_PostConditions(ws, op.drv_sid, op.read_objs, op.tds_dst_src, op.fds_dst_src, op.dos_dst_src, ws', ws_d) &&
                            (ws', ws_d) == DM_GreenDrvRead_InnerFunc(ws, op.drv_sid, op.read_objs, op.tds_dst_src, op.fds_dst_src, op.dos_dst_src); (ws', ws_d)
    ...
    else if (op.DM_DrvActivateToRedPartitionOp?) then
        var ws', ws_d :| DM_Common_PostConditions(ws, ws', ws_d) && (ws', ws_d) == DM_DrvActivateToRedPartition_InnerFunc(ws, op.drv_sid); (ws', ws_d) 
    ...
    else if (op.DM_RedDrvDeactivateOp?) then
        var ws', ws_d :| DM_Common_PostConditions(ws, ws', ws_d) && (ws', ws_d) == DM_RedDrvDeactivate_InnerFunc(ws, op.drv_sid); (ws', ws_d)
    ...
    else if (op.DM_ExternalObjsActivateToRedPartitionOp?) then
        var ws', ws_d :| DM_Common_PostConditions(ws, ws', ws_d) && 
                         (ws', ws_d) == DM_ExternalObjsActivateToRedPartition_InnerFunc(ws, op.td_ids, op.fd_ids, op.do_ids); (ws', ws_d)
    else if (op.DM_GreenExternalObjsDeactivateOp?) then
        var ws', ws_d :| DM_Common_PostConditions(ws, ws', ws_d) &&
                        (ws', ws_d) == DM_GreenExternalObjsDeactivate_InnerFunc(ws, op.td_ids, op.fd_ids, op.do_ids, op.green_pid); (ws', ws_d)
    else
        var ws', ws_d :| DM_Common_PostConditions(ws, ws', ws_d) &&
                        (ws', ws_d) == DM_RedExternalObjsDeactivate_InnerFunc(ws, op.td_ids, op.fd_ids, op.do_ids); (ws', ws_d)
}
