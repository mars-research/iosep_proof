...
datatype DM_Op   =  DM_RedDrvReadOp(drv_sid:Subject_ID, read_objs:set<Object_ID>, tds_dst_src:map<TD_ID, TD_ID>, fds_dst_src:map<FD_ID, FD_ID>, dos_dst_src:map<DO_ID, DO_ID>) | 
                    ...
                    DM_DrvActivateToRedPartitionOp(drv_sid:Subject_ID) |
                    DM_GreenDrvDeactivateOp(drv_sid:Subject_ID) | 
                    DM_RedDrvDeactivateOp(drv_sid:Subject_ID) | 
                    ...
                    DM_ExternalObjsActivateToRedPartitionOp(td_ids:set<TD_ID>, fd_ids:set<FD_ID>, do_ids:set<DO_ID>) | 
                    DM_GreenExternalObjsDeactivateOp(td_ids:set<TD_ID>, fd_ids:set<FD_ID>, do_ids:set<DO_ID>, green_pid:Partition_ID) |
                    DM_RedExternalObjsDeactivateOp(td_ids:set<TD_ID>, fd_ids:set<FD_ID>, do_ids:set<DO_ID>)
    ...
    requires op.DM_GreenDrvReadOp?
{
    ...
    ==> (exists ws':DM_State, ws_d:bool :: DM_GreenDrvRead_PostConditions(ws, op.drv_sid, op.read_objs, op.tds_dst_src, op.fds_dst_src, op.dos_dst_src, ws', ws_d) &&
                                (ws', ws_d) == DM_GreenDrvRead_InnerFunc(ws, op.drv_sid, op.read_objs, op.tds_dst_src, op.fds_dst_src, op.dos_dst_src))
}
...
predicate P_DM_OpsProperties_DrvActivateToRedPartitionOp(ws:DM_State, op:DM_Op)
    requires op.DM_DrvActivateToRedPartitionOp?
{
    (DM_IsValidState(ws) && DM_IsSecureState(ws) &&
            Drv_ID(op.drv_sid) in ws.subjects.drvs
    )
    ==> (exists ws':DM_State, ws_d:bool :: DM_Common_PostConditions(ws, ws', ws_d) && (ws', ws_d) == DM_DrvActivateToRedPartition_InnerFunc(ws, op.drv_sid))
}
...
predicate P_DM_OpsProperties_RedDrvDeactivateOp(ws:DM_State, op:DM_Op)
    requires op.DM_RedDrvDeactivateOp?
{
    (DM_IsValidState(ws) && DM_IsSecureState(ws) &&
        Drv_ID(op.drv_sid) in ws.subjects.drvs &&
        DM_SubjPID(ws.subjects, op.drv_sid) == ws.red_pid &&
    ...
    ==> (exists ws':DM_State, ws_d:bool :: DM_Common_PostConditions(ws, ws', ws_d) && (ws', ws_d) == DM_RedDrvDeactivate_InnerFunc(ws, op.drv_sid))
}
...
predicate P_DM_OpsProperties_ExternalObjsActivateToRedPartitionOp(ws:DM_State, op:DM_Op)
    requires op.DM_ExternalObjsActivateToRedPartitionOp?
{
    DM_ExternalObjsActivateToRedPartition_PreConditions(ws, op.td_ids, op.fd_ids, op.do_ids)
    ==> (exists ws':DM_State, ws_d:bool :: DM_Common_PostConditions(ws, ws', ws_d) && 
                            (ws', ws_d) == DM_ExternalObjsActivateToRedPartition_InnerFunc(ws, op.td_ids, op.fd_ids, op.do_ids))
}
...
predicate P_DM_OpsProperties_RedExternalObjsDeactivateOp(ws:DM_State, op:DM_Op)
    requires op.DM_RedExternalObjsDeactivateOp?
{
    DM_RedExternalObjsDeactivate_PreConditions(ws, op.td_ids, op.fd_ids, op.do_ids)
    ==> (exists ws':DM_State, ws_d:bool :: DM_Common_PostConditions(ws, ws', ws_d) &&
                            (ws', ws_d) == DM_RedExternalObjsDeactivate_InnerFunc(ws, op.td_ids, op.fd_ids, op.do_ids))
}
...
)
{
    ...
    ((forall td_id :: td_id in td_id_val_map ==> td_id in AllTDIDs(ws.objects)) &&
    ...
    requires DM_RedDevWrite_PreConditions(ws, dev_sid, td_id_val_map, fd_id_val_map, do_id_val_map)
{
    ...
    (
        (forall td_id :: td_id in td_id_val_map ==> td_id in ws.objects.active_non_hcoded_tds) &&
        ...
        // Property: Written TDs, FDs and DOs are in the I/O state. 
...
)
{
    ...
    (td_ids <= AllTDIDs(ws.objects)) &&
    (fd_ids <= AllFDIDs(ws.objects)) &&
    (do_ids <= AllDOIDs(ws.objects)) &&
...
predicate DM_ExternalObjsActivateToRedPartition_PreConditions(
    ws:DM_State,
...
predicate DM_RedExternalObjsDeactivate_PreConditions(
    ws:DM_State,
    td_ids:set<TD_ID>, fd_ids:set<FD_ID>, do_ids:set<DO_ID>
)
{
    ...
    P_RedDevsHaveNoTransferToGivenRedObjsAtAnyTime(ws, td_ids, fd_ids, do_ids) &&
