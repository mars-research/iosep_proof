...
lemma Lemma_NewDM_SubjObjDeactivate_FulfillSI2(
    ...
    requires forall td_id :: td_id in DM_TDIDsInGreen(ws)
        ...
        // Properties can be proved with other preconditions, but hard to be discovered by Dafny
    requires (forall td_id :: td_id in DM_TDIDsInGreen(ws)
                ...
                ==> DoTDsIncludeSecureNoTDWriteTransfersOnly(KToKParams(k'), ActiveTDsState(k'), td_id))
{
    if(pid == ws.red_pid)
    {
        Lemma_DM_TDIDsInGreen_ProveEqual(ws, ws');
...
lemma Lemma_NewDM_DevDeactivate_OtherDevsHaveUnchangedPIDs(
    ...
    requires forall id :: id in ws.subjects.devs[wimpdev_id].td_ids && 
                    id != ws.subjects.devs[wimpdev_id].hcoded_td_id 
                 ==> id in ws.objects.active_non_hcoded_tds
    requires forall id :: id in ws.subjects.devs[wimpdev_id].fd_ids ==> id in ws.objects.active_fds
    requires forall id :: id in ws.subjects.devs[wimpdev_id].do_ids ==> id in ws.objects.active_dos
...
//******** Common Lemmas ********//
lemma Lemma_DM_DrvDeactivate_ObjectsToBeDeactivatedAreInSamePartitionWithDrv(
...
lemma Lemma_DM_ObjsDeactivate_ProveCheckOfObjDeactivateInK(
    ...
    ensures todeactivate_td_ids <= AllTDIDs(k.objects)
    ensures todeactivate_fd_ids <= AllFDIDs(k.objects)
    ensures todeactivate_do_ids <= AllDOIDs(k.objects)
...
lemma Lemma_DM_DrvDeactivate_ProveCheckOfDrvDeactivateInK_IfDrvIsInRed(
    ws:DM_State, k:State, drv_sid:Subject_ID
)
    requires DM_IsValidState(ws) && DM_IsSecureState(ws)
    requires k == DMStateToState(ws)

    requires Drv_ID(drv_sid) in DM_AllDrvIDs(ws.subjects)
    requires Drv_ID(drv_sid) in k.subjects.drvs
    requires drv_sid in DM_AllSubjsIDs(ws.subjects)

    requires DM_SubjPID(ws.subjects, drv_sid) == ws.red_pid
        // Requirement: The driver is in red
    requires P_RedDevsHaveNoTransferToGivenRedDrvAtAnyTime(ws, Drv_ID(drv_sid))
        // Requirement: No red device has transfers to any objects of the 
        // driver to be deactivated

    ensures IsValidState(k)
    ensures P_DrvDeactivate_ReturnTrue_Def(k, drv_sid)
{
    // Prove IsValidState(k)
    Lemma_DMStateToState_SecureK(ws, k);
    assert IsValidState(k) && IsSecureState(k);
    ...
    forall o_id, dev_id2 | o_id in OwnObjIDs(k, drv_sid) && 
                dev_id2 in AllActiveDevs(k)
        ensures forall tds_state2 :: tds_state2 in AllReachableActiveTDsStates(k)
                    ==> DevAModesToObj(k, tds_state2, dev_id2, o_id) == {}
    {
        Lemma_OwnObjIDs_Property(k, drv_sid);
        assert DoOwnObj(k, drv_sid, o_id);
        Lemma_ObjsInSubjsAreAlsoInState(k);
        assert o_id in AllObjsIDs(k.objects);
        
        Lemma_K_SubjAndItsObjHasSamePID(k, drv_sid, o_id, ws.red_pid);
        assert ObjPID(k, o_id) == ws.red_pid;
            ...
            ensures DevAModesToObj(k, tds_state2, dev_id2, o_id) == {}
        {
            ...
            assert DM_SubjPID(ws.subjects, drv_sid) == ws.red_pid;
            
            assert DM_AllActiveDevs(ws.subjects) == AllActiveDevs(k);
            if(dev_id2 in DM_DevsInRed(ws))
            {
                assert dev_id2 in DM_DevsInRed(ws);
                assert P_RedDevsHaveNoTransferToGivenRedDrvAtAnyTime(ws, drv_id);
                assert DevAModesToObj(k, tds_state2, dev_id2, o_id) == {};
            }
            else
            {
    ...
    Lemma_DM_DrvDeactivate_ProveCheckOfRedDrvDeactivateInK_Summary(k, drv_sid);
}
    ...
    ensures P_ExternalObjsDeactivate_ReturnTrue_Def(k, todeactivate_td_ids, todeactivate_fd_ids, todeactivate_do_ids, green_pid)
{
    ...
    assert forall id :: id in todeactivate_td_ids ==> id in AllTDIDs(k.objects);
    assert forall id :: id in todeactivate_fd_ids ==> id in AllFDIDs(k.objects);
    assert forall id :: id in todeactivate_do_ids ==> id in AllDOIDs(k.objects);
    
    Lemma_SetInclusion_Prove(todeactivate_td_ids, AllTDIDs(k.objects));
    Lemma_SetInclusion_Prove(todeactivate_fd_ids, AllFDIDs(k.objects));
    Lemma_SetInclusion_Prove(todeactivate_do_ids, AllDOIDs(k.objects));
...
lemma Lemma_DM_RedExternalObjsDeactivate_ProveCheckOfExternalObjsDeactivateInK(
    ws:DM_State, k:State, 
    ...
    ensures IsValidState(k)
    ensures P_ExternalObjsDeactivate_ReturnTrue_Def(k, td_ids, fd_ids, do_ids, ws.red_pid)
{
    // Prove IsValidState(k)
    Lemma_DMStateToState_SecureK(ws, k);
    assert IsValidState(k) && IsSecureState(k);
    ...
    Lemma_DM_RedExternalObjsDeactivate_ProveCheckOfExternalObjsDeactivateInK_Summary(k, td_ids, fd_ids, do_ids, objs);
    assert P_ObjsDeactivate_ReturnTrue_Def(k, td_ids, fd_ids, do_ids);
}
...
lemma Lemma_DM_RedExternalObjsDeactivate_ProveCheckOfExternalObjsDeactivateInK_Summary(
    k:State, td_ids:set<TD_ID>, fd_ids:set<FD_ID>, do_ids:set<DO_ID>, objs:set<Object_ID>
)
    requires IsValidState(k)
    ...
    requires objs == TDIDsToObjIDs(td_ids) + FDIDsToObjIDs(fd_ids) + DOIDsToObjIDs(do_ids)
    requires forall o_id, dev_id2 :: o_id in objs && 
                dev_id2 in AllActiveDevs(k)
                ==> (forall tds_state2 :: tds_state2 in AllReachableActiveTDsStates(k)
                        ==> DevAModesToObj(k, tds_state2, dev_id2, o_id) == {})
    
    ensures P_ObjsDeactivate_ReturnTrue_Def(k, td_ids, fd_ids, do_ids)
{
    // Dafny can automatically prove this lemma
}

lemma Lemma_DM_DrvDeactivate_ProveCheckOfRedDrvDeactivateInK_Summary(
    k:State, drv_sid:Subject_ID
)
    requires IsValidState(k)
    requires P_ObjsInSubjsAreAlsoInState(k)

    requires IsDrvID(k, Drv_ID(drv_sid))
    requires SubjPID(k, drv_sid) != NULL

    requires forall o_id, dev_id2 :: o_id in OwnObjIDs(k, drv_sid) && 
                dev_id2 in AllActiveDevs(k)
                ==> (forall tds_state2 :: tds_state2 in AllReachableActiveTDsStates(k)
                        ==> DevAModesToObj(k, tds_state2, dev_id2, o_id) == {})
    ...
    ensures P_DrvDeactivate_ReturnTrue_Def(k, drv_sid)
{
    ...
    var todeactivate_td_ids := k.subjects.drvs[Drv_ID(drv_sid)].td_ids;
    var todeactivate_fd_ids := k.subjects.drvs[Drv_ID(drv_sid)].fd_ids;
    var todeactivate_do_ids := k.subjects.drvs[Drv_ID(drv_sid)].do_ids;

    assert OwnObjIDs(k, drv_sid) == TDIDsToObjIDs(todeactivate_td_ids) + FDIDsToObjIDs(todeactivate_fd_ids) + DOIDsToObjIDs(todeactivate_do_ids);
}